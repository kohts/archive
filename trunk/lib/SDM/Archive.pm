...
our $authors_canonical = {
    ...
    "Топчиев А.В." => [],
    "Трофимов В.В." => [ "Трофимов, Вадим Вадимович", {"name" => "Trofimov, Vadim", "lang" => "en"} ],
    "Трушагина" => [],
    ...
    "Carpenter C.R." => [ {"name" => "Carpenter, Clarence Ray", "lang" => "en"} ],
...
sub find_author {
    ...
    if (defined($SDM::Archive::authors_canonical->{$author})) {
        ...
        foreach my $author_short (keys %{$SDM::Archive::authors_canonical}) {
            if (
                $equal_with_alterations->($author_short, $canonical_search_author) ||
                $equal_with_alterations->($author_short, $author)
                ) {
            ...
            foreach my $author_struct (@{$SDM::Archive::authors_canonical->{$author_short}}) {
                ...
                foreach my $a (@{$check}) {
                    if ($equal_with_alterations->($a, $author)) {
                        $push_found_author->($author_short);
                    }
                }
            }
