...
#            Carp::confess("Unable to run [$IOW::Test::user_config_path]") unless $return;
    ...
    foreach my $st_gr_id (keys %{$data_desc_struct->{'AE'}->{'storage_groups'}}) {
        Carp::confess("storage group name must be unique, defined twice: " .
            $data_desc_struct->{'AE'}->{'storage_groups'}->{$st_gr_id}->{'name'})
            if $data_desc_struct->{'storage_groups_by_name'}->{$data_desc_struct->{'AE'}->{'storage_groups'}->{$st_gr_id}->{'name'}};
        
        $data_desc_struct->{'AE'}->{'storage_groups'}->{$st_gr_id}->{'id'} = $st_gr_id;
        $data_desc_struct->{'storage_groups_by_name'}->{$data_desc_struct->{'AE'}->{'storage_groups'}->{$st_gr_id}->{'name'}} =
            $data_desc_struct->{'AE'}->{'storage_groups'}->{$st_gr_id};
        
        foreach my $fund_id (@{$data_desc_struct->{'AE'}->{'storage_groups'}->{$st_gr_id}->{'funds'}}) {
...
sub match_classification_group_by_code {
    ...
    my $found_cc_group;
    foreach my $cc (keys %{$data_desc_struct->{'AE'}->{'storage_groups'}->{'1'}->{'classification_codes'}}) {
        my $tmp_cc = $cc;
        $tmp_cc =~ s/[\.,\(\)]//g;
