#!/usr/bin/perl
#
# This is a utility program is intended to convert tab separated dump
...
# and mass updated of metadata in external system with consequent
...
# which will updated already existing items.
#
#
...
if ($o->{'dump-data-desc'}) {
    ...
    TSV_LINE: foreach my $line (@{$list}) {
        ...
        if ($fund_number) {
            $in_doc_struct->{'storage_items_by_fund_number'}->{$fund_number} = {}
                unless $in_doc_struct->{'storage_items_by_fund_number'}->{$fund_number};
            $in_doc_struct->{'storage_items_by_fund_number'}->{$fund_number}->{$st_id} = $storage_struct;
        }
    }
